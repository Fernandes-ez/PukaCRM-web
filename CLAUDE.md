...
atribuição). Camada de `services/` espelhando 1:1 os módulos do backend
(`employeeService`, `roleService`, `leadService`, `conversationService`,
`assistantService`, `whatsappInstanceService`, `workScheduleService`).
...
## Backend pronto, sem UI ainda (pode ser construído)

- **Assinatura/cobrança** (gateway Asaas) — `GET /subscription` +
  `PATCH /subscription/plan` já existem e estão testados no backend
  (decisão arquitetural #14 do `CLAUDE.MD` do backend). Nenhuma tela
  consome isso ainda. `SubscriptionRead` tem `plan`/`status`/
  `trial_ends_at`/`current_period_end`/`cancel_at_period_end` — nunca
  expõe os IDs internos do Asaas.
